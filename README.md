...
**現時点では組み込んでいない。** 公式アカウントが一次情報を出すことは多いが、
API は有料枠が必要で、未認証のスクレイピングはサイト側が遮断している。
代替の公開ミラーも実用に耐えない。当面は、公式アカウントの発表を引用した
ニュース記事を RSS と検索で拾う形で間接的にカバーする。
X の API キーを用意できるなら、`aew/sources.py` に読み込み口を足せる。
